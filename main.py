...
class BingCreatorImageDownload:
    ...
    async def __download_and_zip_images(self) -> None:
        ...
        """
        logging.info(f"Starting download of {len(self.__image_data)} images.")
        zip_name = f"bing_images_{date.today()}.zip"
        with zipfile.ZipFile(zip_name, "w") as zip_file:
            async with tempfile.TemporaryDirectory() as temp_dir:
                tasks = [
                    self.__download_and_save_image(image_dict, temp_dir)
                    for image_dict in self.__image_data
                ]
                file_names = await asyncio.gather(*tasks)
                file_names = list(filter(None, file_names))
                for file_name, collection_name in file_names:
                    new_file_name = await self.__handle_existing_file(file_name)
                    zip_file.write(new_file_name, arcname=os.path.join(collection_name, os.path.basename(new_file_name)))

    async def __handle_existing_file(self, file_name):
        count = 1
        new_file_name = file_name
        while os.path.exists(new_file_name):
            file_name_parts = os.path.splitext(file_name)
            new_file_name = f"{file_name_parts[0]} ({count}){file_name_parts[1]}"
            count += 1
